...
writer = SummaryWriter(os.path.join(opt.save_path, 'summary'), flush_secs=30)
...
models_path = os.path.join(opt.save_path, "models")
if not os.path.exists(models_path):   
    os.makedirs(models_path)

logging.basicConfig(filename=os.path.join(opt.save_path, "log", datetime.now().strftime("%Y-%m-%d-%H_%M_%S") + ".log"),
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        ...
        torch.save(model.state_dict(), os.path.join(save_path, "models", 'MSEDNET_Last.pth'))
       
        with open("./run/plot/train_loss.txt", 'w') as train_los:
        ...
        torch.save(model.state_dict(), os.path.join(save_path, "models", 'MSEDNET_Inter.pth'))
    ...
    with torch.no_grad():   
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae  
                best_epoch = epoch  
                torch.save(model.state_dict(), os.path.join(save_path, "models", 'MSEDNET_Best.pth'))
